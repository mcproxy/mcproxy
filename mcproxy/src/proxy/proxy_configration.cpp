...
#include <cctype>
#include <locale>
proxy_configuration::proxy_configuration(const shared_ptr<interfaces> interfaces)
...
bool proxy_configuration::load_config(const std::string& path)
{
    HC_LOG_TRACE("");
...
bool proxy_configuration::parse_config(const std::string& path)
{
    HC_LOG_TRACE("");
    ...
    if (!file) {
        ...
        while (!file.eof()) {
                ...
                do {
                    ...
                    if (!comp_str.empty() && comp_str.at(0) != '#') {
                        ...
                        if (comp_str.compare("protocol") == 0) {
                            ...
                            tmp_upstream_if = m_interfaces->get_if_index(comp_str);
                            ...
                            tmp_downstream_if = m_interfaces->get_if_index(comp_str);
            ...
            if (state == LOOK_FOR_ADDITIONAL_DOWNSTREAM) {
                //m_upstream_downstream_map.insert(upstream_downsteram_pair(tmp_upstream_if, tmp_downstream_set));
                if (!add_upstream(tmp_upstream_if)) {
                    return false;
                }
               
                for(auto e: tmp_downstream_set){
                    if(!add_downstream(tmp_upstream_if,e)){
                        return false;                                  
                    }
                } 
...
bool proxy_configuration::check_double_used_if(unsigned int if_index)
{
    HC_LOG_TRACE("");
    for (auto & e : m_upstream_downstream_map) {
        if (e.first == if_index) {
            ...
            if (e.second.find(if_index) != end(e.second)) {
                return false;
            }
        }
    }
...
{
    HC_LOG_TRACE("");
    if (check_double_used_if(m_upstream_downstream_map, if_index_downstream)) {

        if (m_interfaces->is_interface_up(if_index_downstream)) {
...
{
    HC_LOG_TRACE("");
    if (check_double_used_if(m_upstream_downstream_map, if_index_upstream)) {

        if (m_interfaces->is_interface_up(if_index_upstream)) {
...
bool proxy_configuration::del_upstream(unsigned int if_index_upstream)
{
    ...
    auto it = m_upstream_downstream_map.find(if_index_upstream);
    if (it != end(m_upstream_downstream_map)) {
        m_upstream_downstream_map.erase(it);
        return true;
    } else {
        return false;
    }
...
const upstream_downstream_map& proxy_configuration::get_upstream_downstream_map()
{
    HC_LOG_TRACE("");
