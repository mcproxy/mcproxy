...
bool proxy_instance::init(int addr_family, int version, int upstream_index, int upstream_vif, int downstream_index,
                          int downstram_vif, bool single_instance)
{
    HC_LOG_TRACE("");
...
bool proxy_instance::init_receiver()
{
    ...
    if (!m_receiver->init(m_addr_family, m_version, &m_mrt_sock)) {
        return false;
    }
    m_receiver->start();
        ...
        if (r->if_index == m_upstream) {
            ...
            if (it_gss == m_upstream_state.end()) { //new group found
                ...
                if (iter_src == it_gss->second.end()) { //new src addr
                    ...
                    HC_LOG_DEBUG("kernel msg with src: " << r->src_addr << " received, this source address exist for if_index: " <<
                                 r->if_index << " and group:" << r->g_addr << " because it's normaly not joined on a downstream interface");
                    return;
                }
            }
                ...
                if (db->get_level_of_detail() > debug_msg::MORE) {
                    if (tmp_src_state_map->size() > 0 ) {
                        ...
                        for (iter_src_state = tmp_src_state_map->begin(); iter_src_state != tmp_src_state_map->end(); iter_src_state++) {
                            str << "\t\t[" << tmp_s_counter++ << "] " << iter_src_state->first << "\t"  << iter_src_state->second.robustness_counter
                                << "\t" << iter_src_state->second.state_type_to_string() << endl;
                        }
                    }
                }
            }
        ...
        for (iter_table = m_state_table.begin(); iter_table != m_state_table.end(); iter_table++) {
            ...
            if (db->get_level_of_detail() > debug_msg::NORMAL) {
                ...
                for (iter_state = iter_table->second.begin(); iter_state != iter_table->second.end(); iter_state++) {
                    ...
                    str << "\t\t[" << tmp_g_counter++ << "] " << iter_state->first << "\t"  << tmp_gsp->second.robustness_counter << "\t" <<
                        tmp_gsp->second.state_type_to_string() << endl;
                    ...
                    if (db->get_level_of_detail() > debug_msg::MORE) {
                        if (tmp_gsp->first.size() > 0 ) {
                            ...
                            for (iter_src_state = tmp_gsp->first.begin(); iter_src_state != tmp_gsp->first.end(); iter_src_state++) {
                                str << "\t\t\t[" << tmp_s_counter++ << "] " << iter_src_state->first << "\t"  <<
                                    iter_src_state->second.robustness_counter << "\t" << iter_src_state->second.state_type_to_string() << endl;
                            }
                        }
                    }
                }
...
bool proxy_instance::split_traffic(int if_index, const addr_storage& g_addr, const addr_storage& src_addr)
{
    ...
    if (vif_list.size() == 0) {
        return false;    //if nobody join this group ignore
    }
...
void proxy_instance::add_all_group_vifs_to_list(std::list<unsigned int>& vif_list, int without_if_index,
        addr_storage g_addr)
{
    ...
    for (iter_table = m_state_table.begin(); iter_table != m_state_table.end(); iter_table++) {
        if (without_if_index != iter_table->first) {
            ...
            if (iter_state != iter_table->second.end()) {
                ...
                if (sgs_pair->second.flag == src_state::RUNNING || sgs_pair->second.flag == src_state::RESPONSE_STATE
                        || sgs_pair->second.flag == src_state::WAIT_FOR_DEL) {
